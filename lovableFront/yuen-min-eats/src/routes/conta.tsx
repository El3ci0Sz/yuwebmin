...
export const Route = createFileRoute("/conta")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Gerencie seu perfil, endereço e cartão fidelidade no YuWebMin." },
    ],
  }),
  component: ContaPage,
});
    ...
    return (
      <SiteLayout>
        <section className="mx-auto max-w-md px-4 py-20 text-center">
          ...
          <Button asChild className="mt-6 rounded-full"><Link to="/login">Entrar</Link></Button>
        </section>
      </SiteLayout>
    );
  }
  ...
  return (
    <SiteLayout>
      <section className="mx-auto max-w-5xl px-4 pt-10 pb-16">
        <div className="rounded-3xl p-8 text-primary-foreground" style={{ background: "linear-gradient(135deg, var(--primary), oklch(0.82 0.14 70))" }}>
          <div className="flex flex-wrap items-center gap-4">
            ...
            <Button variant="secondary" className="rounded-full" onClick={() => { store.logout(); navigate({ to: "/" }); }}>
        ...
        <div className="mt-6 grid md:grid-cols-2 gap-4">
          <div className="rounded-2xl bg-card border border-border p-6">
            ...
            <div className="rounded-xl p-5 text-card" style={{ background: "var(--gradient-warm)" }}>
              <div className="text-xs uppercase tracking-wider text-foreground/60">Pontos acumulados</div>
              <div className="text-4xl font-bold text-foreground mt-1">{user.points}</div>
              <div className="text-xs text-foreground/70 mt-2">Faltam {Math.max(0, 200 - user.points)} pontos para o próximo brinde 🎁</div>
            </div>
            <div className="mt-3 h-2 rounded-full bg-secondary overflow-hidden">
              <div className="h-full bg-primary transition-all" style={{ width: `${Math.min(100, (user.points / 200) * 100)}%` }} />
            </div>
          </div>
          ...
          <div className="rounded-2xl bg-card border border-border p-6">
            ...
            <form onSubmit={saveAddress} className="space-y-3">
              <div>
                ...
                <Input id="address" value={address} onChange={(e) => setAddress(e.target.value)} placeholder="Ex: Rua das Flores, 123 - Centro" />
              </div>
              <Button type="submit" className="rounded-full">Salvar endereço</Button>
            </form>
          </div>
        </div>
